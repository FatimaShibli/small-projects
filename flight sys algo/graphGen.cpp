...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < num_nodes; i++)
    {
        ...
        num_neighbors = myRand(num_nodes/3, 10);
    ...
    fstream testFile;
    testFile.open("flight.txt", ios::out);
    for (i = 0; i < num_nodes; i++)
    {
        if (!graph[i].neighbors.size())
            continue;
        cout << "From:  " << city[graph[i].nodeID] << endl
             << "To  : ";
        testFile << "From:  " << city[graph[i].nodeID] << endl
             << "To  : ";
        for (j = 0; j < graph[i].neighbors.size(); j++)
        {
            if (j>0)
            {
                ...
                testFile << "       " << city[graph[i].neighbors[j]] << endl;
            }
            else{
                cout << " " << city[graph[i].neighbors[j]] << endl;
                testFile << " " << city[graph[i].neighbors[j]] << endl;
